...
def run_flask():
    print("🌐 Starting Flask server on http://0.0.0.0:6003")
    app.run(host="0.0.0.0", port=6003, debug=False, use_reloader=False, threaded=True)
...
def publish_current_pose():
    try:
        ...
        current_robot_message = json.dumps({"robot": current_robot})
        redis_client.publish("robot_3", current_robot_message)
    except Exception as e:
        print(f"Redis publish error: {e}")
...
print("   - Flask API server at http://localhost:6003/control")
if opc_ua_started:
    print("   - OPC UA server at opc.tcp://0.0.0.0:4843/freeopcua/server/")
